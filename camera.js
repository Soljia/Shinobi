...
console.log("FFMPeg Dir")
console.log(config.ffmpegDir);
...
s.video = function(x, e, k) {
    ...
    switch (x) {
        case 'fix':
            ...
            e.spawn = spawn(config.ffmpegDir, ('-i ' + e.dir + e.filename + ' ' + e.fixFlags + ' ' + e.sdir + e.filename).split(' '), { detached: true }).on("error", (error) => {
...
case 'insertCompleted':
    ...
    s.ffmpeg = function(e) {
        ...
        return spawn(config.ffmpegDir, x.ffmpegCommandString, { detached: true, stdio: x.stdioPipes }).on("error", (error) => {
            console.log("Error: " + error);
            console.log(config.ffmpegDir);
            throw error;
        })
                        ...
                        if (e.mon.details.snap === '1') {
                            ...
                            switch (e.mon.type) {
                                ...
                                case 'local':
                                    ...
                                    e.spawn = spawn(config.ffmpegDir, ('-loglevel quiet -i ' + e.url + ' -s 400x400 -r 25 -ss 1.8 -frames:v 1 -f singlejpeg pipe:1').split(' '), { detached: true }).on("error", (error) => {
                                        console.log("Error: " + error);
                                        console.log(config.ffmpegDir);
                                        throw error;
                                    })
                            ...
                            case 'system':
                                switch (d.ff) {
                                    case 'update':
                                        ...
                                        var updateProcess = spawn('sh', (__dirname + '/UPDATE.sh ' + d.distro).split(' '), { detached: true }).on("error", (error) => {
                                            console.log("Error: " + error);
                                            console.log(config.ffmpegDir);
                                            throw error;
                                        })
                    ...
                    switch (d.f) {
                        ...
                        case 'created_file':
                            ...
                            fs.writeFile(d.dir + d.filename, d.created_file, 'binary', function(err, data) {
                                ...
                                s.tx({ f: 'video_build_success', filename: s.group[d.d.ke].mon[d.d.mid].open + '.' + s.group[d.d.ke].mon[d.d.mid].open_ext, mid: d.d.mid, ke: d.d.ke, time: s.nameToTime(s.group[d.d.ke].mon[d.d.mid].open), end: s.moment(new Date, 'YYYY-MM-DD HH:mm:ss') }, 'GRP_' + d.d.ke);
                            });
                            break;
                    }
                }
    ...
    app.get(['/:auth/grid/:ke', '/:auth/grid/:ke/:group'], function(req, res) {
        ...
        s.auth(req.params, function(user) {
            ...
            req.sql = 'SELECT * FROM Monitors WHERE mode!=? AND mode!=? AND ke=?';
            req.ar = ['stop', 'idle', req.params.ke];
            if (!req.params.id) {
    ...
    app.get(['/:auth/tvChannels/:ke', '/:auth/tvChannels/:ke/:id', '/get.php'], function(req, res) {
        ...
        req.fn = function(user) {
            ...
            req.sql = 'SELECT * FROM Monitors WHERE mode!=? AND ke=?';
            req.ar = ['stop', req.params.ke];
            if (!req.params.id) {
                if (user.details.sub && user.details.monitors && user.details.allmonitors !== '1') {
                    try { user.details.monitors = JSON.parse(user.details.monitors); } catch (er) {}
                    req.or = [];
                        ...
                        s.sqlQuery('SELECT * FROM Monitors WHERE ke=? AND mid=?', [req.monitor.ke, req.monitor.mid], function(er, r) {
                            ...
                            if (req.finish === 1) {
                                ...
                                if (req.monitor.mode === 'stop') {
                                    ...
                                    s.camera('stop', req.monitor);
                                    setTimeout(function() { s.camera(req.monitor.mode, req.monitor); }, 5000)
                                };
    ...
    app.get(['/:auth/fileBin/:ke', '/:auth/fileBin/:ke/:id'], function(req, res) {
        ...
        res.header("Access-Control-Allow-Origin", req.headers.origin);
        req.fn = function(user) {
            req.sql = 'SELECT * FROM Files WHERE ke=?';
            req.ar = [req.params.ke];
            if (user.details.sub && user.details.monitors && user.details.allmonitors !== '1') {
                try { user.details.monitors = JSON.parse(user.details.monitors); } catch (er) {}
                req.or = [];
