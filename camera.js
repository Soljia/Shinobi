...
s.video = function(x, e, k) {
    ...
    switch (x) {
        case 'fix':
            ...
            e.spawn = spawn(config.ffmpegDir, ('-i ' + e.dir + e.filename + ' ' + e.fixFlags + ' ' + e.sdir + e.filename).split(' '), { detached: true, env: { PATH: process.env.PATH } })
...
s.ffmpeg = function(e) {
    ...
    return spawn(config.ffmpegDir, x.ffmpegCommandString, { detached: true, stdio: x.stdioPipes, env: { PATH: process.env.PATH } });
}
s.file = function(x, e) {
    if (!e) { e = {} };
    switch (x) {
                    ...
                    if (e.mon.details.snap === '1') {
                        ...
                        switch (e.mon.type) {
                            ...
                            case 'local':
                                ...
                                e.spawn = spawn(config.ffmpegDir, ('-loglevel quiet -i ' + e.url + ' -s 400x400 -r 25 -ss 1.8 -frames:v 1 -f singlejpeg pipe:1').split(' '), { detached: true, env: { PATH: process.env.PATH } })
                                e.spawn.stdout.on('data', function(data) {
                        ...
                        case 'system':
                            switch (d.ff) {
                                case 'update':
                                    ...
                                    var updateProcess = spawn('sh', (__dirname + '/UPDATE.sh ' + d.distro).split(' '), { detached: true, env: { PATH: process.env.PATH } })
