(function(global, factory) {
  ...
  c3_chart_fn.xgrids.add = function(grids) {
    var $$ = this.internal;
    return this.xgrids($$.config.grid_x_lines.concat(grids || []));
  };
  ...
  c3_chart_fn.ygrids.add = function(grids) {
    var $$ = this.internal;
    return this.ygrids($$.config.grid_y_lines.concat(grids || []));
  };
  ...
  c3_chart_internal_fn.getSvgArcExpanded = function(rate) {
    var $$ = this,
      arc = $$.d3.svg.arc().outerRadius($$.radiusExpanded * (rate || 1)).innerRadius($$.innerRadius);
    return function(d) {
 ...
 */
  ...
  c3_chart_internal_fn.updateXDomain = function(targets, withUpdateXDomain, withUpdateOrgXDomain, withTrim, domain) {
    ...
    if (withUpdateOrgXDomain) {
      $$.x.domain(domain || $$.d3.extent($$.getXDomain(targets)));
      $$.orgXDomain = $$.x.domain();
      if (config.zoom_enabled) {
    ...
    if (withUpdateXDomain) {
      $$.x.domain(domain || (!$$.brush || $$.brush.empty() ? $$.orgXDomain : $$.brush.extent()));
      if (config.zoom_enabled) {
        $$.zoom.scale($$.x).updateScaleExtent();
      }
    }
  ...
  c3_chart_internal_fn.generateGetBarPoints = function(barIndices, isSub) {
    ...
    return function(d, i) {
      ...
      if ($$.config.axis_rotated) {
        if (d.value > 0 && posY < y0 || d.value < 0 && y0 < posY) {
          posY = y0;
        }
      }
      // 4 points that make a bar
  ...
  c3_chart_internal_fn.getTextRect = function(text, cls, element) {
    var dummy = this.d3.select('body').append('div').classed('c3', true),
      ...
      rect;
    svg.selectAll('.dummy').data([text]).enter().append('text').classed(cls || '', true).style('font', font).text(text).each(function() {
