    ...
    sql.select('*').from('Videos').where({ mid: e.mid, ke: e.ke, time: misc.nameToTime(filename) }).asCallback(
      (error, rows) => {
        if (rows && rows[0]) {
          ...
          sql('Videos').where({ mid: e.mid, ke: e.ke, time: misc.nameToTime(filename) }).del().then(function () {
            fs.stat(dir + filename, function (err, file) {
              if (err) {
                winston.log({ level: 'error', message: "File Delete Error (" + e.ke + ":" + e.mid + ") " + err.toString() })
