    ...
    module.init = function() {
        ...
        app.get('/:auth/logout/:ke/:id', function(req, res) {
            if (s.group[req.params.ke] && s.group[req.params.ke].users[req.params.auth]) {
                ...
                sql.query("UPDATE Users SET auth=? WHERE auth=? AND ke=? AND uid=?", ['', req.params.auth, req.params.ke, req.params.id])
                            ...
                            if (req.body.pass === req.body.password_again) {
                                sql.query('SELECT * FROM Users WHERE mail=?', [req.body.mail], function(err, r) {
                                    if (r && r[0]) { //found one exist
                                        ...
                                        sql.query('INSERT INTO Users (ke,uid,mail,pass,details) VALUES (?,?,?,?,?)', [req.params.ke, req.gid, req.body.mail, misc.md5(req.body.pass), req.body.details])
                            ...
                            sql.query('SELECT * FROM Monitors WHERE ke=? AND mid=?', [req.monitor.ke, req.monitor.mid], function(er, r) {
                                ...
                                if (req.finish === 1) {
                                    ...
                                    if (req.monitor.mode === 'stop') {
                                        ...
                                        camera.camera('stop', req.monitor);
                                        setTimeout(function() { camera.camera(req.monitor.mode, req.monitor); }, 5000)
                                    };
        ...
        app.get('/:auth/videos/:ke/:id/:file', function(req, res) {
            s.auth(req.params, function(user) {
                ...
                sql.query('SELECT * FROM Videos WHERE ke=? AND mid=? AND time=?', [req.params.ke, req.params.id, misc.nameToTime(req.params.file)], function(err, r) {
                    if (r && r[0]) {
        ...
        app.get(['/:auth/videos/:ke/:id/:file/:mode', '/:auth/videos/:ke/:id/:file/:mode/:f'], function(req, res) {
                ...
                s.auth(req.params, function(user) {
                    ...
                    sql.query(req.sql, req.ar, function(err, r) {
                        if (r && r[0]) {
                            r = r[0];
                            r.filename = misc.moment(r.time) + '.' + r.ext;
                            switch (req.params.mode) {
                                ...
                                case 'status':
                                    ...
                                    if (isNaN(req.params.f) || req.params.f === 0) {
                                        ...
                                        req.ret.ok = true;
                                        sql.query('UPDATE Videos SET status=? WHERE ke=? AND mid=? AND time=?', [req.params.f, req.params.ke, req.params.id, misc.nameToTime(req.params.file)])
