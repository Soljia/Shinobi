...
module.exports = function(vars) { //s,config,misc,logging){
    ...
    output.init = function(req, res) {
        ...
        if (req.body.mail && req.body.pass) {
            req.default = function() {
                sql.query('SELECT * FROM Users WHERE mail=? AND pass=?', [req.body.mail, misc.md5(req.body.pass)], function(err, r) {
                    req.resp = { ok: false };
                    if (!err && r && r[0]) {
                        r = r[0];
                        r.auth = misc.md5(misc.gid());
