    ...
    output.init = function(cn) {
        ...
        cn.on('f', function(d) {
            if (!cn.ke && d.f === 'init') { //socket login
                ...
                d.failed = function() {
                    misc.tx({ ok: false, msg: 'Not Authorized', token_used: d.auth, ke: d.ke });
                    cn.disconnect();
                }
                d.success = function(r) {
                    r = r[0];
                                    ...
                                    switch (d.fff) {
                                        case 'videos&events':
                                            ...
                                            var getEvents = function(callback) {
                                                ...
                                                sql.query(eventQuery, eventQueryValues, function(err, r) {
                                                    if (err) {
                                            ...
                                            var getVideos = function(callback) {
                                                ...
                                                sql.query(videoQuery, videoQueryValues, function(err, r) {
                                                    if (err) {
                ...
                if (d.f === 'init') {
                    ...
                    if (s.cron && cn.id === s.cron.id) {
                        ...
                        switch (d.f) {
                            ...
                            case 'end':
                                d.mid = '_cron';
                                logging.log(d, { type: 'cron', msg: d.msg })
                                break;
                            default:
        ...
        cn.on('a', function(d) {
                if (!cn.init && d.f == 'init') {
                    sql.query('SELECT * FROM Users WHERE auth=? AND uid=?', [d.auth, d.uid], function(err, r) {
                        if (r && r[0]) {
                            r = r[0];
                    ...
                    switch (d.f) {
                        ...
                        case 'created_file':
                            ...
                            fs.writeFile(d.dir + d.filename, d.created_file, 'binary', function(err, data) {
                                ...
                                misc.tx({ f: 'delete_file', file: d.filename, ke: d.d.ke, mid: d.d.mid });
                                misc.tx({ f: 'video_build_success', filename: s.group[d.d.ke].mon[d.d.mid].open + '.' + s.group[d.d.ke].mon[d.d.mid].open_ext, mid: d.d.mid, ke: d.d.ke, time: misc.nameToTime(s.group[d.d.ke].mon[d.d.mid].open), end: misc.moment(new Date, 'YYYY-MM-DD HH:mm:ss') }, 'GRP_' + d.d.ke);
                            });
                            break;
                    }
                }
